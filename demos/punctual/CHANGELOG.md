...
| Coordination | Durable Object serialises, then re-checks Google/Microsoft | Durable Object `Calendar` serialises `book()` / `cancel()`; no external calendar |
| Cache | KV availability | KV `CACHE`; delete on book **and** cancel |
...
| Auth / OAuth | Google Calendar, Microsoft 365, signing keys | HMAC signing for ICS/cancel; Bearer admin key. No Google/Microsoft |
| Extra products | Teams, event types, REST, MCP, webhooks, embed | Cut |
| Host config | Dashboard + OAuth calendars | Wrangler vars (`HOST_*`, hours, timezone) |
...
- Google Calendar and Microsoft 365 OAuth / free-busy
...
## What we kept (simplified) — grow slice
